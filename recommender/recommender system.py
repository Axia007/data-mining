...
if case == 1:
    ...
    testRDD3 = testRDD2.map(lambda x:calWUserbased(x))
elif case == 3:
    for item in bustouser:
        av = bustouser[item].values()
        average[item] = sum(av) / len(av)
    testRDD3 = testRDD2.map(lambda x: calWItembased(x,case))
elif case == 4:
    for item in bustouser:
        av = bustouser[item].values()
        average[item] = sum(av) / len(av)
...
result = testRDD3.map(lambda x : (x[0][0],x[0][1],x[1])).map(lambda x: (reuserId[x[0]],reubusId[x[1]], x[2])).collect()
time_end = time.time()
